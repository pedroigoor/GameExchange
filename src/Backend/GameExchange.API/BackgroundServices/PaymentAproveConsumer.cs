...
{
    public class PaymentAproveConsumer(IServiceProvider services,
                                      IEventConsumer eventConsumer) : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            await _eventConsumer.StartConsuming(
                queue: ConstRabbitMQ.PAYMENT_APROVE_QUEUE,
                exchange: ConstRabbitMQ.PAYMENT_EXCHANGE,
                routingKey: ConstRabbitMQ.PAYMENT_APROVE_ROUTING_KEY,
                async (json) =>
                {                    
